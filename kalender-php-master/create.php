...
if ( isset( $_GET[ 'id' ] ) ) {
    $id = $_GET[ 'id' ];
    $sql = "SELECT person, day, month, year FROM birthdays WHERE id=" . $id;
...
<head>
    ...
    <title>Maak een verjaardag aan</title>
</head>

<body>
<h1>Maak een verjaardag aan</h1>
<form class="editForm" action="create-action.php" method="post">
    ...
    <br>
    <label for="month">Maand:</label> <input type="text" name="month" id="month" maxlength="2" <?php echo $row[ "month" ] ?>" value="<?php echo $row[ "month" ]  ?>" required>*
    <br>
